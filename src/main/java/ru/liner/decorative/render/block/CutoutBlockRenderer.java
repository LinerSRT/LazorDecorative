...
public class CutoutBlockRenderer implements ISimpleBlockRenderingHandler {
    ...
    private boolean renderBlock(IBlockAccess world, int x, int y, int z, Block block, RenderBlocks renderer) {
        ...
        boolean connectedToTop = wasConnected(blockPosition, EnumFacing.UP, (ICutoutRenderer) block);
        boolean connectedToBottom = wasConnected(blockPosition, EnumFacing.DOWN, (ICutoutRenderer) block);
        ...
        if(connectedToTop && connectedToBottom)
            topCutout = 0;
        double eastCutout = wasConnected(blockPosition, EnumFacing.EAST, (ICutoutRenderer) block) ? 0 : ((ICutoutRenderer) block).getCutoutSize(EnumFacing.EAST);
        double westCutout = wasConnected(blockPosition, EnumFacing.WEST, (ICutoutRenderer) block) ? 0 : ((ICutoutRenderer) block).getCutoutSize(EnumFacing.WEST);
        double northCutout = wasConnected(blockPosition, EnumFacing.NORTH, (ICutoutRenderer) block) ? 0 : ((ICutoutRenderer) block).getCutoutSize(EnumFacing.NORTH);
        double southCutout = wasConnected(blockPosition, EnumFacing.SOUTH, (ICutoutRenderer) block) ? 0 : ((ICutoutRenderer) block).getCutoutSize(EnumFacing.SOUTH);
    ...
    private boolean wasConnected(Vector3 position, EnumFacing facing, ICutoutRenderer cutoutRenderer) {
        return cutoutRenderer.allowRenderConnection(position.offset(facing).blockId());
    }
